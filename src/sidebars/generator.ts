...
- ${Object.keys(docsById).join('\n- ')}`,
   ...
   */
  function treeify(tutorials: SidebarItemsGeneratorDoc[]): Dir {
  ...
  function generateSidebar(
    ...
    function createCategoryItem(
      ...
      function findDocByLocalId(localId: string): SidebarItemDoc | undefined {
        return allItems.find(
          (item): item is SidebarItemDoc =>
            item.type === 'tutorial' && getLocalDocId(item.id) === localId,
        );
      }
      ...
      const items = allItems.filter(
        (item) => !(item.type === 'tutorial' && item.id === categoryLinkedDoc?.id),
      );
