...
function toSidebarsProp(loadedVersion) {
    const docsById = (0, docs_1.createDocsByIdIndex)(loadedVersion.tutorials);
...
function toVersionDocsProp(loadedVersion) {
    return Object.fromEntries(loadedVersion.tutorials.map((tutorial) => [
...
function toTagDocListProp({ allTagsPath, tag, tutorials, }) {
    function toDocListProp() {
        const list = lodash_1.default.compact(tag.tutorialIds.map((id) => tutorials.find((doc) => doc.id === id)));
        // Sort docs by title
