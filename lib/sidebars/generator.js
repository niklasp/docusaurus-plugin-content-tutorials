...
const DefaultSidebarItemsGenerator = ({ numberPrefixParser, isCategoryIndex, tutorials: allDocs, item: { dirName: autogenDir }, categoriesMetadata, }) => {
    ...
    function generateSidebar(fsModel) {
        ...
        function createCategoryItem(dir, fullPath, folderName) {
            ...
            function findDocByLocalId(localId) {
                return allItems.find((item) => item.type === 'tutorial' && getLocalDocId(item.id) === localId);
            }
            ...
            function getCategoryLinkedDocMetadata() {
                ...
                if (link !== undefined && link?.type !== 'tutorial') {
            ...
            const items = allItems.filter((item) => !(item.type === 'tutorial' && item.id === categoryLinkedDoc?.id));
